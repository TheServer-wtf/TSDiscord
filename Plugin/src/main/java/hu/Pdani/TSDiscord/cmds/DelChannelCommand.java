...
public class DelChannelCommand implements ProgramCommand {
    ...
    public void run(InteractionOriginalResponseUpdater builder, List<SlashCommandInteractionOption> options) {
        ...
        if(!channels.contains(serverChannel.getIdAsString())){
            builder.setContent("The provided channel: "+serverChannel.getName()+" ("+serverChannel.getIdAsString()+"); is not a valid "+typeOption.getStringRepresentationValue().get()+" channel!").update();
            return;
        }
        ...
        builder.setContent("Removed channel "+serverChannel.getName()+" ("+serverChannel.getIdAsString()+") from being used as a "+typeOption.getStringRepresentationValue().get()+" channel").update();
    }
    ...
    public List<SlashCommandOption> getOptions() {
        return List.of(
            SlashCommandOption.createWithChoices(SlashCommandOptionType.STRING, "type", "The type of channel to remove", true,
                    List.of(
